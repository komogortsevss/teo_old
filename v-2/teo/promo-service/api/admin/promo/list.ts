...
export const apiAdminPromoListRoute = app.get("/", async (ctx, req) => {
  ...
  const promos: PromoListItem[] = promoCodes.map((promo) => ({
    ...
    allowedProducts: promo.allowedProducts
      ? JSON.parse(promo.allowedProducts)
      : undefined,
    excludedProducts: promo.excludedProducts
      ? JSON.parse(promo.excludedProducts)
      : undefined,
